...
#' model across multiple experimental conditions. Each dataset contains binary
#' responses from 10 groups to 10 items under a 2-Parameter Logistic (2PL) IRT
#' model with varying levels of differential item functioning (DIF).
#'
#' @section Overview:
#' The script performs the following operations:
#' 1. Sets up population parameters for 10 distinct groups
...
#'
#' DIF magnitude patterns across 10 groups:
#' - Groups 1-2: No DIF (reference groups)
#' - Groups 3-4: Moderate DIF (a: ±0.5, b: ±1)
#' - Groups 5-6: Moderate DIF with reversed signs
#' - Groups 7-8: Large DIF (a: ±1, b: ±1.5)
#' - Groups 9-10: Large DIF with reversed signs
#'
#' @section Group Structure:
#' - **Number of groups**: 10
#' - **Group size ratios**: Unequal (10%, 10%, 15%, 15%, 5%, 5%, 15%, 15%, 5%, 5%)
#' - **Ability distributions**: Groups differ in mean ability (μ: -1, 0, or 1)
#' - **Variance**: σ² = 1 for all groups
#'
#' @section Experimental Conditions:
#' The script generates data across multiple conditions:
#' - **Sample sizes**: 1000, 2000, 4000 respondents per dataset
#' - **DIF percentages**: 20%, 40%, 60% (2, 4, or 6 items with DIF out of 10)
#' - **Replications**: 250 independent datasets per condition
#' - **Total datasets**: 3 sample sizes × 3 DIF% × 250 replications = 2,250 files
#'
...
#' - Format: "Updated_Ten_Group_Testing_Data_[N]_[perc]_Replication_[r].RData"
#' - Contains:
...
#' # source("Ten_Group_Testing_Data_Generation.R")
...
for (N in c(1000, 2000, 4000)) {  # Sample sizes to test
  for (m in c(2, 4, 6)) {  # Number of DIF items (20%, 40%, 60%)
    for (r in 1:250) {  # Replications per condition
      ...
      save.image(
        file = paste0(
          "Updated_Ten_Group_Testing_Data_", N, "_", m * 10,
