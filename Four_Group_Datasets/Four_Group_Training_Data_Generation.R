...
n_vals <- 250:2000
valid_n <- n_vals[n_vals %% 10 == 0] # Make sure we have integer group sizes
weights <- ifelse(valid_n < 1000, 3, 1)  # 3 times more likely if  N < 1000
weights <- weights / sum(weights)

for(r in 1:1000){ # Number of data sets to generate
  set.seed(202407)
  ...
  # Randomly draw for DIF Shift between groups
  dif.a = sample(grid.a, size = S, replace = T)
  dif.a[c(1,2)] = 0 # Reference groups
  dif.b = sample(grid.b, size = S, replace = T)
  dif.b[c(1,2)] = 0 # Reference groups
