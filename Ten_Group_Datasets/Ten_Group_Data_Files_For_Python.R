...
# Results.SIB = as.matrix(Results.SIB)
# Results.SIB = as.numeric(Results.SIB)
# Results.SIB = matrix(Results.SIB, nrow = 10)
...
for (rep in 1:500) {
  ...
  write_csv(out.ALL.features, paste0("Ten_Group_Training_data_ALL_Replication",training_file_counter,".csv"))
  write_csv(out.TLP.features, paste0("Ten_Group_Training_data_TLP_Replication",training_file_counter,".csv"))
  write_csv(out.labels, paste0("Ten_Group_Training_labels_Replication",training_file_counter,".csv"))
...
for (N in c(1000, 2000, 4000)) {
  for (p in c(20, 40)) {
    testing_file_counter <- 1
    for (r in 1:500) {
      file_path <- paste0('Estimated_Ten_Group_Testing_data_',N,'_',p,'_Replication',r,'.RData')
      if (!file.exists(file_path)) {
        next
      }
      load(file_path)
      ...
      write_csv(out.ALL.features, paste0("Ten_Group_Testing_data_ALL_",N,"_",p,"_Replication",testing_file_counter,".csv"))
      write_csv(out.TLP.features, paste0("Ten_Group_Testing_data_TLP_",N,"_",p,"_Replication",testing_file_counter,".csv"))
      write_csv(out.labels, paste0("Ten_Group_Testing_data_labels_",N,"_",p,"_Replication",testing_file_counter,".csv"))
